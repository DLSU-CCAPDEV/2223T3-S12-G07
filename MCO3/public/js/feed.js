...
function render_downvote(button){
    ...
    if(downvote.length>0){
        $(downvote).attr("class","downvote_icon_true");
        $(text).css("color","#B70C0C");
        votes.downvotes+=1;
        var upvote = $(button).parent().find('.upvote_icon_true');
        
        if(upvote!=null){
            $(upvote).attr("class","upvote_icon");
            var upvote_text = $(upvote).parent().find('.actions');
            $(upvote_text).css("color" , "#797878");
            votes.upvotes-=1;
        }
    }else{
        downvote = button.children('.downvote_icon_true');
        $(downvote).attr("class","downvote_icon");
...
function render_upvote(button){
    ...
    var votes={downvotes: 0, upvotes: 0, button_id: `${$(button).attr('id')}`};
    if(upvote.length>0){
        $(upvote).attr("class","upvote_icon_true");
        $(text).css("color", "#0C6DB7");
        votes.upvotes +=1;
        var downvote = $(button).parent().find('.downvote_icon_true');
        if(downvote!=null){
            $(downvote).attr("class","downvote_icon");
        ...
        upvote= $(button).children('.upvote_icon_true');
        $(upvote).attr("class","upvote_icon");
        $(text).css("color","#797878");       
        votes.upvotes -=1; 
    }
